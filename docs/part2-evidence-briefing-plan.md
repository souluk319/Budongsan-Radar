...
2026-06-14 기준 공개 홈은 샘플 집픽이 아니라 실제 수집 데이터가 먼저 보이도록 전환했다.

- `npm run data:live` 추가
- 네이버 뉴스 검색 API에서 부동산 정책/대출/청약/전세/매매시장 후보를 수집
- 수집 링크를 `status=published`, `is_sample=false`로 저장
- 샘플 링크의 `is_daily_pick`을 해제해 공개 첫 화면이 샘플로 시작하지 않게 조정
- 실제 published links 8개 확인
- 실제 data observations 10개 이상 확인
- 홈 첫 화면에 `실제 수집 8건`, 실제 뉴스 제목, `공식 근거 3개` 노출 확인
- 현재 OpenAI 요약은 운영 속도를 위해 기본 비활성화. `LIVE_USE_OPENAI=1`을 넣으면 live ingest에서 OpenAI structured summary를 시도한다.

주의: 법제처 API는 현재 `필수입력요소 검증에 실패하였습니다` 응답을 반환하므로 정상 근거로 쓰지 못하고 warning으로만 남긴다. REB/R-ONE도 아직 실제 통계 endpoint 매핑 전이다.
